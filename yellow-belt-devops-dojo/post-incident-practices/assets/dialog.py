...
def setcolors():
    ...
    curses.init_pair(11, 136, curses.COLOR_BLACK)
...
def redraw_on_resize(stdscr, dialog_id):
    ...
    for dialog2 in dialogs2:
        ...
        stdscr.addstr(" ")
        stdscr.addstr(textwrap.fill(dialog2['text'].replace("[student]", firstname), width - len(dialog2['speaking']) - 3),
                      curses.color_pair(color(dialog2['speaking'])))
        stdscr.addstr("\n")
...
def main(stdscr):
    ...
    # Open the dialog
    try:
        dialogs = yaml.load_all(open(dialog_file, 'r'), Loader=yaml.FullLoader)
